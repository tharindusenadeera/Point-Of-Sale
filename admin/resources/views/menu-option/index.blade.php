...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Menu Options
            <small>All the Menu options</small>
        </h1>
        <ol class="breadcrumb">
                    ...
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $menuOption->name }}</td>
                                    <td class="text-center">
                                        @if ($menuOption->status == 1)
                                        <small class="label  bg-green">Active</small>
                                        @elseif ($menuOption->status == 0)
                                        <small class="label  bg-yellow">Inactive</small>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <button type="button" onclick="getData({{$menuOption->id}})"
                                            class="btn  btn-info btn-xs ">Edit/ View</button>

                                            @if ($menuOption->status == 1)
                                                <button type="button" onclick="toggleStatus(this)" data-status="0"  data-id="{{$menuOption->id}}"  class="btn  btn-warning btn-xs ">Inactivate</button>
                                            @elseif ($menuOption->status ==  0)
                                                <button type="button"  onclick="toggleStatus(this)" data-status="1"  data-id="{{$menuOption->id}}"  class="btn  btn-success btn-xs ">Activate</button>
                                            @endif
                                    </td>
                                </tr>
                                @endforeach
...
@endsection
@include('menu-option.menu_option')
@section('aditionalJs')
  <script type="text/javascript">
    // start use for request js
    baseUrl           = "{{url("menu-option")}}";
    url               = baseUrl;
    primaryKey        = "id"; // use to set update url
    reloadAfterSubmit = true;
    formId = "menuOptionForm";
    // useSwal =1;
    // end use for request js
    function showForm() {
        resetForm("menuOptionForm");
